...
export const SpeakersPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', phone: '', email: '', spiritistCenter: '', observations: '' });
  ...
  return (
    <div className="p-6 lg:p-10 space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl lg:text-4xl font-black text-gray-900 tracking-tighter italic">Palestrantes</h1>
          <p className="text-gray-500 font-medium tracking-tight">Gestão de irmãos que compartilham a palavra no Mirante de Luz</p>
        </div>
        ...
        {filtered.length > 0 ? filtered.map((s, index) => (
          ...
          >
            ...
            <div className="space-y-1 mb-6">
              <h3 className="text-xl font-black text-gray-900 group-hover:text-indigo-600 transition-colors">{s.name}</h3>
            ...
            <div className="grid grid-cols-1 gap-3 text-sm text-gray-500 font-medium border-t border-gray-50 pt-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="truncate font-bold text-gray-700">{s.email}</span>
              </div>
              ...
              {s.observations && (
                <div className="mt-2 p-4 bg-gray-50/50 rounded-2xl border border-gray-100/50">
                  ...
                  <p className="text-xs italic leading-relaxed text-gray-400 line-clamp-2">{s.observations}</p>
                </div>
              )}
            </div>
          </motion.div>
          ...
          <div className="col-span-full py-20 text-center space-y-4">
            ...
            <p className="text-gray-400 font-bold italic tracking-tight">Nenhum palestrante encontrado.</p>
          </div>
        )}
      </div>
            ...
            >
              <div className="p-8 pb-4 border-b border-gray-50 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs font-medium text-gray-400">Gerencie as informações do irmão palestrante</p>
                </div>
                <button 
                  onClick={() => {
                    setIsModalOpen(false);
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Centro Espírita de Origem</label>
                  <input
                    required
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button type="submit" className="flex-[1.5] py-3.5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 group">
