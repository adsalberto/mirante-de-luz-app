...
export const ParticipantsPage: React.FC = () => {
  ...
  const loadParticipants = async () => {
    ...
    setParticipants(p);
    setActiveQueues(q.filter(entry => entry.status !== 'FINISHED' && entry.status !== 'CANCELLED'));
  };

  const handleEdit = (p: Participant) => {
    if (['RECEPCIONISTA', 'ATENDENTE', 'VOLUNTARIO'].includes(currentUser?.role || '')) {
      alert('Ação não permitida: Seu perfil atual não possui privilégios para esta operação.');
      return;
    }
          ...
          <div className="bg-gray-100 p-1 rounded-2xl flex items-center shadow-inner">
            ...
            {(currentUser?.role === 'ADMIN' || currentUser?.role === 'ADM' || (currentUser?.position && ['Presidente(s)', 'Vice-presidente(s)', '1º Secretário(a)', 'Secretário(a) de Planejamento'].includes(currentUser.position))) && (
              <button
                ...
                className={cn(
                  "px-4 py-2 rounded-xl text-xs font-black uppercase transition-all",
                  activeTab === 'referrals' ? "bg-white text-indigo-600 shadow-sm" : "text-gray-400 hover:text-gray-600"
                )}
              >
                Encaminhamentos
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.length > 0 ? filtered.map((p) => (
              ...
              >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={cn(
                          ...
                          p.currentStatus === 'IDLE' ? "bg-gray-50 text-gray-400 border-gray-100" :
                          p.currentStatus === 'WAITING' ? "bg-amber-50 text-amber-600 border-amber-100" :
                          p.currentStatus === 'IN_SERVICE' ? "bg-indigo-50 text-indigo-600 border-indigo-100" :
                          p.currentStatus === 'COMPLETED' ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
                          "bg-blue-50 text-blue-600 border-blue-100"
                        )}>
                          {p.currentStatus === 'IDLE' ? 'Livre' : 
                           p.currentStatus === 'WAITING' ? 'Em Espera' :
                           p.currentStatus === 'IN_SERVICE' ? 'Em Atendimento' :
                           p.currentStatus === 'COMPLETED' ? 'Concluído' :
                           p.currentStatus === 'REFERRERED' ? 'Encaminhado' : p.currentStatus}
                        </span>
                        <span className={cn(
                          "text-[10px] font-black uppercase px-2 py-0.5 rounded-full tracking-wider border",
                      ...
                      <>
                        ...
                        <button 
                          ...
                          className={cn(
                            ...
                            deletingId === p.id ? "bg-red-500 text-white text-[10px] font-bold px-3 py-1" : "text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100"
                          )}
                        >
                          {deletingId === p.id ? "Confirma?" : <Trash2 size={18} />}
                        </button>
                      </>
                    )}
                  </div>
                ...
                <div className="space-y-2 text-sm text-gray-500 font-medium">
                  ...
                  <div className="flex items-center gap-2 pt-2 text-[10px]">
                    ...
                    <span className="text-emerald-600 font-bold uppercase tracking-tight">LGPD: Aceito em {format(p.lgpdDate, 'dd/MM/yyyy')}</span>
                  </div>
                  
                  {/* Active Service Notification Area */}
                  {activeQueues.filter(q => q.participantId === p.id).length > 0 && (
                    <div className="pt-2">
                      <div className="p-2 bg-amber-50 rounded-xl border border-amber-100 flex items-center gap-2 text-amber-700 text-[10px] font-black uppercase tracking-tighter">
                        ...
                        Atendimento Ativo: {
                          activeQueues
                            ...
                            .map(aq => sectors.find(s => s.id === aq.sectorId)?.name)
                            .filter(Boolean)
                            .join(', ')
                        }
                      </div>
              ...
              <div className="col-span-full py-20 text-center space-y-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-400">Nenhum resultado</h3>
                  <p className="text-gray-400">Tente buscar por outro termo ou cadastre um novo atendido.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
             <div className="flex items-center gap-3">
               <button 
                 ...
                 className={cn(
                   ...
                   encaminhamentoFilter === 'all' ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-500 border-gray-100 hover:border-indigo-100"
                 )}
               >
                 Todos
               </button>
               <button 
                 ...
                 className={cn(
                   "px-4 py-2 rounded-xl text-xs font-bold transition-all border",
                   encaminhamentoFilter === 'Doutrinária' ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-500 border-gray-100 hover:border-indigo-100"
                 )}
               >
                 Doutrinária
               </button>
             ...
             <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">
               Total: {evolutions.filter(e => encaminhamentoFilter === 'all' || e.encaminhamento === encaminhamentoFilter).length} registros
             </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-50 overflow-hidden shadow-sm">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50">
                {evolutions
                  .filter(e => encaminhamentoFilter === 'all' || e.encaminhamento === encaminhamentoFilter)
                  .map(evo => {
                    const participant = participants.find(p => p.id === evo.participantId);
                    const sector = sectors.find(s => s.id === evo.sectorId);
                    return (
                      <tr key={evo.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <div className="font-bold text-gray-900">{participant?.name || 'Não identificado'}</div>
                          <div className="text-[10px] text-gray-400 font-mono tracking-tighter uppercase">ID: {evo.participantId.substring(0, 8)}</div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                             ...
                             {(evo.nextStepSectorIds || []).map(sid => {
                               const targetSector = sectors.find(s => s.id === sid);
                               return (
                                 <span key={sid} className="px-2 py-1 bg-amber-50 text-amber-600 rounded-lg text-[10px] font-black uppercase border border-amber-100">
            ...
            {evolutions.filter(e => encaminhamentoFilter === 'all' || e.encaminhamento === encaminhamentoFilter).length === 0 && (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-white rounded-[32px] shadow-2xl overflow-hidden shadow-indigo-900/20"
            >
              <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">{editingParticipant ? 'Editar Cadastro' : 'Novo Cadastro'}</h2>
                  <p className="text-sm text-gray-500 font-medium">Preencha os dados com fraternidade e atenção.</p>
                </div>
                <button 
                  id="close-modal-btn"
                  onClick={() => {
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Endereço de Residência</label>
                    <textarea
                      required
                      rows={2}
                ...
                <div className="p-6 bg-indigo-50/50 rounded-[32px] border border-indigo-100 flex gap-4">
                  ...
                  <div className="space-y-1">
                    <label htmlFor="lgpd-consent" className="text-sm font-bold text-indigo-900 cursor-pointer">Termo de Consentimento (LGPD)</label>
                    <p className="text-xs text-indigo-700/70 font-medium leading-relaxed">
                      Eu autorizo o Centro Espírita Mirante de Luz a tratar meus dados pessoais e sensíveis (especialmente anotações de cunho espiritual) para fins de tratamento e acompanhamento fraterno, em conformidade com a Lei Geral de Proteção de Dados.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <span>{editingParticipant ? 'Salvar Alterações' : 'Salvar Cadastro'}</span>
