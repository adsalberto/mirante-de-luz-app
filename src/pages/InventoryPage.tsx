...
export const InventoryPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        console.log(
          "Bootstrap empty inventory database with standard CEMIL items mapped to sectors...",
        );
        ...
        const livrariaId =
          getSectorIdByName("Comunicação") ||
          getSectorIdByName("Administrativo");
        ...
        const bootstrapItems: Omit<InventoryItem, "id">[] = [
          ...
          },
          {
            ...
            status: "REGULAR",
            observation:
              "Estoque atual insuficiente para a festa de final do mês",
            lastUpdated: Date.now(),
            updatedBy: currentUser?.name || "Sistema",
          },
          {
            ...
            status: "BOM",
            observation:
              "Impressão da ficha cadastral de atendimentos e avisos",
            lastUpdated: Date.now(),
            updatedBy: currentUser?.name || "Sistema",
          },
          {
  ...
  const getStatusBadge = (status: InventoryItemStatus) => {
    switch (status) {
      ...
      case "EM_MANUTENCAO":
        return (
          <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-105 animate-pulse">
            🛠 Manutenção
          </span>
        );
      case "EM_FALTA":
        return (
          <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-gray-150 text-gray-650 border border-gray-250 line-through">
            🛑 Acabou
          </span>
        );
      default:
  ...
  const getSectorIcon = (sectorName: string) => {
    ...
    if (s.includes("admin"))
      return <Building2 className="shrink-0" size={16} />;
  ...
  const getSectorsStatsCount = () => {
    return Array.from(new Set(items.map((i) => i.sectorId).filter(Boolean)))
      .length;
  };
    ...
    <div className="p-6 lg:p-10 space-y-8 animate-in fade-in duration-500">
            ...
            <button
              ...
              onClick={() => {
                ...
                setFormData({
                  ...
                  location: "",
                  sectorId:
                    selectedSectorId !== "ALL" && selectedSectorId !== "NONE"
                      ? selectedSectorId
                      : "",
                  status: "BOM",
                  observation: "",
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        >
          ...
          <div className="mt-2 text-xs font-semibold text-gray-500">
            {countLowStockInSector(selectedSectorId) > 0
              ? "⚠️ Necessita de compra/reposição"
      ...
      <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm space-y-4">
        ...
        <div className="flex items-center justify-between border-t border-gray-50 pt-4 flex-wrap gap-2 text-xs">
          <label className="inline-flex items-center gap-3 cursor-pointer select-none">
            ...
            />
            <span className="font-extrabold text-gray-550 tracking-tight">
              Esconder itens em estoque satisfatório (Mostrar apenas reposição
              necessário)
            </span>
          </label>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center p-24 gap-4">
            ...
            <p className="text-indigo-600 text-[10px] font-black uppercase tracking-widest animate-pulse">
              Sincronizando inventário integrado por setores...
            </p>
          </div>
        ) : filtered.length === 0 ? (
                  ...
                  return (
                    <tr
                      ...
                      className={cn(
                        ...
                        isLow
                          ? "border-l-rose-500/80 bg-rose-50/10"
                    ...
                    >
                      ...
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {item.observation ? (
                              <div
                                className="text-gray-400 text-[10px] mt-0.5 max-w-[220px]"
                      ...
                      <td className="py-5 px-4">
                        <div className="flex items-center gap-1.5 text-indigo-950 font-black text-[11px]">
                          ...
                          <span
                            className="truncate max-w-[140px] text-indigo-950"
                            title={itemSectorName}
                          >
                            {itemSectorName}
                          </span>
                        </div>
      ...
      <footer className="text-center p-4 border-t border-gray-150/50 mt-12 bg-white/20 rounded-3xl text-[10px] text-gray-400 font-extrabold tracking-wider uppercase">
        🔐 Controle de Setores do Mirante de Luz | O inventário é integrado e
        atualizado de forma síncrona com os coordenadores responsáveis
      </footer>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
            <motion.div
              layoutId={
                editingItem ? `item-edit-${editingItem.id}` : "item-new"
              }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-gray-900 leading-none">
                      {editingItem
                        ? "Editar Item do Setor"
                        : "Cadastrar novo bem / insumo"}
                    </h3>
                    <p className="text-gray-400 text-xs font-semibold mt-1">
                      Defina as estatísticas e attribua ao setor correto
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-gray-50 border border-gray-100 p-3 rounded-xl font-bold text-gray-700 outline-none focus:bg-white focus:border-indigo-600 transition-colors text-xs"
                    >
                      {Object.entries(INVENTORY_STATUS_LABELS).map(
                        ([key, label]) => (
                          <option key={key} value={key}>
                            {label}
                          </option>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-gray-400 text-xs font-semibold mt-2 max-w-sm mx-auto">
                ...
                ? Esta ação é irreversível e será gravada nos logs de auditoria.
              </p>
